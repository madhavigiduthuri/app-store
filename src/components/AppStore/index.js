...
class AppStore extends Component {
  state = {
    tabSelected: 'SOCIAL',
    filteredList: appsList.filter(eachApp => eachApp.category === 'SOCIAL'),
    inputValue: '',
  }
  ...
  render() {
    const {tabSelected, filteredList, inputValue} = this.state
      ...
      <div className="appstore-container">
        ...
        <div className="search-container">
          <input
            ...
            value={inputValue}
            onChange={x => this.onChangeInput(x)}
          />
          <img
        ...
        <ul className="tabitem-container">
          {tabsList.map(eachItem => (
            <TabItem
              key={eachItem.tabId}
              tabName={eachItem.displayText}
              selectedTab={tabSelected}
              onChangingTab={() => this.onChangeTab(eachItem.tabId)}
              tabId={eachItem.tabId}
            />
          ))}
        </ul>
        <ul className="appitemstore-container">
          {filteredList.map(eachApp => (
            <AppItem
              key={eachApp.appId}
              appImage={eachApp.imageUrl}
